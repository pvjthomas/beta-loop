...
#   closed -> 0.0, open -> 1.83 (platereader_open's arc endpoint; it then cosmetically
#   commits the 2.0386 URDF full-open preset). We grip and arc between open 1.83 and
#   closed 0.0.
...
def platereader_closed(platereader: SkillObject):
    """Close the plate reader lid by arc-following the ``lid_open`` anchor back down.

    Reverse of platereader_open. Starts with the lid open. Grips the lid at
    ``lid_open`` (resolved with the lid open), then sweeps lid_joint from open (1.83)
    down to closed (0.0), resolving ``lid_open`` at each step so the lid traces its
    real hinge arc with the arm tracking it, committing the joint state as it goes.
    Releases and swings out. Single-anchor version of wellplate_shaker_close.

    Args:
        platereader: The plate reader whose lid to close.
    """
    print_log(runlog=True, runlog_type="step_start")
    print_log("Starting platereader_closed")

    reader_id = platereader.id

    # Model the lid open so the grab lands on the open lid.
    update_object_joint_config(reader_id, {"lid_joint": LID_OPEN_ANGLE})

    # Resolve the grab pose with the lid open; pinch to the anchor's grasp width.
    lid = load_object_anchor(reader_id, "lid_open", joint_config={"lid_joint": LID_OPEN_ANGLE})
    grip_w = lid.get("width", 0.0)
    retreat = load_object_anchor(reader_id, "lid_open_retreat")

    # Swing in via the open-lid retreat pose, approach with the gripper open, pinch.
    ...
    # Arc-follow lid_open from open (1.5) back down to closed (0.0).
    _sweep_lid(reader_id, "lid_open", LID_OPEN_ANGLE, LID_CLOSED_ANGLE, ARC_STEPS, SWING_SPEED)

    # Release the lid, back off through the closed-lid standoff then up (mirroring the
    # open skill's two-step descent), commit, and swing out.
    lid_closed = load_object_anchor(reader_id, "lid_open", joint_config={"lid_joint": LID_CLOSED_ANGLE})
    preapproach = anchor_preapproach(lid_closed, standoff=0.05)
    preapproach_above = [preapproach[0], preapproach[1], preapproach[2] + 0.15]
    set_gripper(arm=ARM, width_m=APPROACH_JAW_M)
    time.sleep(0.3)
    move_arm(arm=ARM, position=preapproach, orientation=lid_closed["rpy"], speed=APPROACH_SPEED, wait=True)
    move_arm(arm=ARM, position=preapproach_above, orientation=lid_closed["rpy"], speed=APPROACH_SPEED, wait=True)
    ...
    print_log("platereader_closed completed")
    return {"success": True}
