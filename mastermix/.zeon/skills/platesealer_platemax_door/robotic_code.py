...
def _tuya_press(cid: str) -> None:
    ...
    try:
        ...
        if isinstance(echoed, dict) and echoed.get(str(DP_SWITCH)) == target:
            print(f"platemax door press: CONFIRMED switch_1 -> {target}")
        else:
            print(f"platemax door press: NO ACK (sent switch_1 -> {target}); reply={result!r}")
    finally:
        device.close()


def platesealer_platemax_door():
    """Press the platemax plate sealer's door button once."""
    if not is_sim_mode():
